...
std::unique_ptr<Expr> Parser::parsePostfix() {
  ...
  while (true) {
    ...
    } else if (current.type == TokenType::PlusPlus ||
               current.type == TokenType::MinusMinus) {
      stream.advance();
...
std::unique_ptr<Expr> Parser::parseUnary() {
  ...
  if (current.type == TokenType::Minus || current.type == TokenType::Exclaim ||
      current.type == TokenType::Tilde || current.type == TokenType::Plus ||
      current.type == TokenType::Star || current.type == TokenType::Ampersand) {
    stream.advance();
...
std::unique_ptr<Statement> Parser::parseStatement() {
  ...
  if (current.type == TokenType::KW_void ||
      ...
      current.type == TokenType::KW_int || current.type == TokenType::KW_long ||
  ...
  if (current.type == TokenType::KW_case ||
      current.type == TokenType::KW_default) {
  ...
  if (current.type == TokenType::KW_struct ||
      ...
      current.type == TokenType::KW_typedef || isTypeSpecifier()) {
    return parseDeclaration();
  }
...
std::unique_ptr<Statement> Parser::parseTypedefDeclaration(
    AttributeList attrs) {
  Token start = stream.current();
  stream.advance();
...
std::unique_ptr<Statement> Parser::parseStructUnionEnumDeclaration(
  ...
  if (stream.current().type == TokenType::KW_struct ||
    ...
    stream.expect(TokenType::Semicolon,
                  "Expected ';' after struct/union declaration");
    return decl;
  }
...
AttributeList Parser::parseAttributes() {
  ...
  while (stream.current().type == TokenType::DobleLBracket) {
    ...
    while (true) {
      ...
      if (stream.current().type != TokenType::Identifier) {
        throw std::runtime_error("Expected attribute name after [[ at line " +
                                 std::to_string(stream.current().line));
      }
...
bool Parser::isTypeSpecifier() {
  ...
  if (type == TokenType::KW_void || type == TokenType::KW_bool ||
      ...
      type == TokenType::KW_restrict || type == TokenType::KW_constexpr ||
      type == TokenType::KW_struct || type == TokenType::KW_union ||
...
EnumConstant Parser::parseEnumConstant() {
  ...
  if (stream.current().type != TokenType::Identifier) {
    throw std::runtime_error(
        ...
        std::to_string(stream.current().column) + " (got '" +
        std::string(stream.current().text) + "')");
  }
