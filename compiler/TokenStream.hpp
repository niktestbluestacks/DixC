 ...
 public:
  ...
  Token expect(TokenType type, const std::string& error_msg) {
    ...
    throw std::runtime_error(
        error_msg + " at line " + std::to_string(current().line) + ", column " +
        std::to_string(current().column) + " (got unknown token)");
  }

  bool isAtEnd() const { return current().type == TokenType::EndOfFile; }

  size_t getPosition() const { return pos; }

  void setPosition(size_t new_pos) { pos = new_pos; }
};
}  // namespace dix
