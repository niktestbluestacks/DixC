...
TokenType Lexer::getTokenType(const std::string_view& str) {
  if (Keywords.contains(str))
    return Keywords[str];
  else if (SpecialSymbols.contains(str))
    return SpecialSymbols[str];
  else if (Preprocessor.contains(str))
    return Preprocessor[str];
...
Token Lexer::scanNumber() {
  ...
  if (last_was_separator) {
    throw std::runtime_error("Digit separator can not be at end of number" +
                             std::to_string(line));
  }
  ...
  if (base == 10 || base == 16) {
    ...
    if (current() == exp_char ||
        current() == (exp_char - 32)) {  // 'p' or 'P', 'e' or 'E'
      is_float = true;
      advance();
      if (current() == '+' || current() == '-') advance();

      if (!std::isdigit(current())) {
        throw std::runtime_error("Missing exponent digits at line " +
                                 std::to_string(line));
      }
...
Token Lexer::scanString() {
  ...
  advance();
  TokenType type =
      (quote == '"') ? TokenType::StringLiteral : TokenType::CharLiteral;
  ...
  while (pos < source.size()) {
    ...
    if (c == '\n') {
      throw std::runtime_error("Newline in string literal at line " +
                               std::to_string(line));
    }
...
Token Lexer::scanOperatorOrPunctuation() {
  ...
  throw std::runtime_error(
      "Unexpected character '" + std::string(1, current()) + "' at line " +
...
Token Lexer::scanPreprocessor() {
  ...
  std::string_view directive =
      source.substr(directive_start, pos - directive_start);
